...
feature1_index = feature_names.index('chroma_stft_mean')
feature2_index = feature_names.index('chroma_stft_var')

plt.figure(figsize=(10, 10))

# Plot each genre as a different color
for genre in np.unique(df['label']):
    plt.scatter(
        df[df['label']==genre]['chroma_stft_mean'], 
        df[df['label']==genre]['chroma_stft_var'], 
        label=genre
    )

# Plot the new song
plt.scatter(
    new_song_features_df['chroma_stft_mean'], 
    new_song_features_df['chroma_stft_var'], 
    c='black', 
    marker='x', 
    label='New Song'
)

# Draw lines from the new song to its K nearest neighbors
distances, indices = knn_model.kneighbors(new_song_features_scaled)
for i in range(knn_model.n_neighbors):
    plt.plot(
        [new_song_features_df['chroma_stft_mean'].iloc[0], df.iloc[indices[0][i]]['chroma_stft_mean']], 
        [new_song_features_df['chroma_stft_var'].iloc[0], df.iloc[indices[0][i]]['chroma_stft_var']], 
        'gray', 
        linestyle='dotted'
    )
...
feature1_index = feature_names.index('rolloff_mean')
feature2_index = feature_names.index('spectral_centroid_mean')
...
for genre in np.unique(df['label']):
    plt.scatter(
        df[df['label']==genre]['rolloff_mean'], 
        df[df['label']==genre]['spectral_centroid_mean'], 
        label=genre
    )

# Plot the new song
plt.scatter(
    new_song_features_df['rolloff_mean'], 
    new_song_features_df['spectral_centroid_mean'], 
    c='black', 
    marker='x', 
    label='New Song'
)
...
for i in range(knn_model.n_neighbors):
    plt.plot(
        [new_song_features_df['rolloff_mean'].iloc[0], df.iloc[indices[0][i]]['rolloff_mean']], 
        [new_song_features_df['spectral_centroid_mean'].iloc[0], df.iloc[indices[0][i]]['spectral_centroid_mean']], 
        'gray', 
        linestyle='dotted'
    )
